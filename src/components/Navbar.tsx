    ...
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-10 flex items-center justify-between">
        ...
        <button
          ...
          className={`lg:hidden ml-auto mr-2 flex items-center gap-1 text-[0.65rem] font-semibold px-2 py-1 rounded-md border border-crimson-700/60 text-crimson-700 ${isScrolled ? "mt-0" : "-mt-4"}`}
        >
        ...
        {/* ─── Desktop Menu ─── */}
        <nav className={`hidden lg:flex items-center gap-8 ${isScrolled ? "" : "-mt-4"}`}>
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
          ...
          <button
            onClick={() => window.dispatchEvent(new CustomEvent("open-birthday-offer"))}
            className="flex items-center gap-1 text-sm font-semibold px-3 py-1.5 rounded-md border border-crimson-700/60 text-crimson-700 hover:bg-crimson-700/5 transition-colors"
          >
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden absolute top-full left-0 w-full h-screen bg-white shadow-xl border-t border-gray-100 py-4 px-6 flex flex-col gap-2 animate-fade-in pb-32">
          ...
          <a
            href="#products"
            onClick={() => setMobileMenuOpen(false)}
            className="bg-[#25D366] text-white text-center font-bold px-6 py-4 rounded-xl shadow-lg shadow-[#25D366]/20 flex items-center justify-center gap-2"
          >
